...
def load_policy_with_reference(model_name: str, adapter: str | Path):
    """One base model carrying two copies of the same adapter (D-017).

    ``policy`` is trainable, ``reference`` is the frozen KL anchor. This is
    numerically identical to loading two separate models from the same
    checkpoint, but stores the 596M base weights once instead of twice, which is
    what makes GRPO fit in 4 GB of VRAM.

    Switch with ``model.set_adapter("policy" | "reference")``.
    """
    dtype = torch.bfloat16 if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else torch.float16
    base = AutoModelForCausalLM.from_pretrained(model_name, dtype=dtype, trust_remote_code=True)
    if torch.cuda.is_available():
        base = base.cuda()
    model = PeftModel.from_pretrained(base, str(adapter), adapter_name="policy", is_trainable=True)
