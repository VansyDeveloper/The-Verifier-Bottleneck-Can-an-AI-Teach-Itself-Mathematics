# Архивные команды прежней серии

Не запускать повторно в рамках нового плана. Пакеты 0.6B и 8B уже получены.
Эта страница сохраняет прежнюю инструкцию для проверки происхождения результатов.
Ссылки внутри прежнего текста относились к корню репозитория.
...
Для 8B парная серия seed 0/1/2 уже найдена в архиве; команды её подготовки:
...
В сравниваемых вариантах сохраняйте одинаковые `--dtype`, `--micro-batch`, `--effective-batch`, `--prefix-batch`. BF16 требует подходящей CUDA-карты. Исторические расходы времени и памяти находятся в `budget.json` соответствующих запусков; профили новой очереди будут сохранены отдельно.
