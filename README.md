...
Проверьте качество отдельных операций в `outputs/atomic_06/eval/summary.json`. Это новая атомарная подготовка; старых весов в Git нет.
...
Команда показывает очередь; `--execute` запускает её. `replay trace` дают 21 расчёт, отдельный `--recipe set` добавляет 9. Для первого замера выберите `--recipe size --seeds 0`. Готовые одинаковые расчёты пропускаются.
...
Верните весь `outputs/`: данные, веса, логи и результаты. Сводка лежит в `results_*.csv`; [команды для H(K)](evidence/README.md), [готовые локальные кривые](evidence/local_series/README.md).
