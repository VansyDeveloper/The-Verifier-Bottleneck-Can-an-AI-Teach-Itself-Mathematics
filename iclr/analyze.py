...
def load_manifest(path):
    ...
    if set(runs) != set(itertools.product(seeds, ARMS)):
        raise ValueError("each seed needs atomic_control and composition")
...
def analyze(manifest, output, plots=False):
    runs, scorer = load_manifest(manifest)
    seeds = sorted({seed for seed, _ in runs})
    ...
    for (split, depth), ids in sorted(groups.items()):
        for k in range(1, 5 ** depth + 1):
            ...
            summaries.append(dict(split=split, depth=depth, k=k, n_seeds=len(seeds), n_tasks=len(ids),
                                  atomic_control=fmean(hits[ARMS[0]]) / len(ids),
                                  composition=fmean(hits[ARMS[1]]) / len(ids), delta=mean,
        ...
        for seed in seeds:
            table = dict(split=split, depth=depth, seed=seed, k=32, n_tasks=len(ids),
                         both=0, control_only=0, composition_only=0, neither=0)
            for task_id in ids:
                ...
                outcome = {(True, True): "both", (True, False): "control_only",
                           (False, True): "composition_only", (False, False): "neither"}[success]
                table[outcome] += 1
                pairs.append(dict(seed=seed, task_id=task_id, **{key: a.get(key) for key in IDENTITY},
                                  control_rank=a["best_rank"], composition_rank=c["best_rank"],
            ...
            table["delta"] = (table["composition_only"] - table["control_only"]) / len(ids)
            counts.append(table)
    ...
    if plots:
        plot_curves(summaries, output)
    return summaries


def plot_curves(rows, output):
    import matplotlib
    matplotlib.use("Agg")
    ...
    for index, ((split, depth), group) in enumerate(sorted(groups.items())):
        ...
        for key, label, color in (("atomic_control", "Atomic control", "#0072B2"),
                                  ("composition", "Composition", "#D55E00")):
            axes[0].plot(ks, [row[key] for row in group], label=label, color=color,
                         linestyle='--' if key == 'atomic_control' else '-')
...
def main():
    ...
    if args.manifest:
        analyze(args.manifest, args.output, args.plots)
    else:
